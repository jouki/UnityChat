...
Promise.all([
  ...
  check('menuModel historie bez userId → neaktivní', mm.menuModel({ history: true, hasUserId: false }).find((i) => i.id === 'history').disabled === true);
  {
    ...
    check('donationsSummary mod: částka + tooltip s odhadem podle jména', dm.amount === `1${NB}250${NB}Kč + 20${NB}€` && dm.guess === true && dm.title === `Celkem darováno 1${NB}250${NB}Kč + 20${NB}€ (z toho 250${NB}Kč jen podle jména)`, JSON.stringify(dm));
    check('donationsSummary divák: jen ucNamed', uh.donationsSummary({ ucNamed: { czk: 1000, count: 1 } }).amount === `1${NB}000${NB}Kč` && uh.donationsSummary({ ucNamed: { czk: 0, count: 0 } }) === null && uh.donationsSummary(undefined) === null && uh.donationsSummary({ count: 0, total: { czk: 0 } }) === null);
