// Detektor odkazů v textu zprávy (moderace část 3 — filtr odkazů, spec
// docs/superpowers/specs/2026-09-25-moderace-odkazy-gify-design.md). Sdílený core:
// žádný DOM, žádné chrome.*. Backend má VĚDOMOU KOPII v backend/src/lib/links.ts
// (Docker image backendu se staví jen z backend/) — při změně upravit obě,
// test backend/src/lib/links.test.ts porovnává výstupy obou na stejných případech.
//
// Co je odkaz:
//  - cokoli se schématem http(s):// / ftp:// (host libovolný, i IP),
//  - bez schématu `www.x.y`, `neco.cz`, `neco.cz/x`, `neco.com:8080` — poslední
//    štítek musí být známá TLD (seznam níže), jinak by `ahoj.jak` nebo `v1.2` byly odkazy.
 ...
 */
export function findLinks(text, opts = {}) {
...
export function linkHosts(text, opts = {}) {
 ...
 */
export function hostAllowed(host, allowDomains) {
