...
- **v3.38.14** - **GQL pin fallback — pin fetch přes Twitch API, nezávislý na DOM**: nejlepší řešení pro "pin se nezobrazuje když chat je schovaný". Nový background handler `FETCH_PINS` volá `channel.pinnedChatMessages` GQL query — vrací strukturovaná data (pinner, sender + chatColor, senderBadges setID/version, content fragments s emote IDs, pinnedAt). Sidepanel `_startPinPoll()` (8s interval) fetchuje pins a pushuje je do highlights banneru jako `kind:'pin'` karty. `_handleHighlights` teď **merguje** DOM-sourced highlights (hype-train/gift/raid) + GQL-sourced pins — DOM pin cards se filtrují out (redundantní). Hide-not-collapse CSS vrácena na stabilní `width: 0` z v3.27.4 — pin data už nezávisí na DOM render state. Channel switch resetuje `_gqlPinCards` + `_lastDomHighlightCards`. **Aktuální verze**
