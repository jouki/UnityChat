...
- **v3.38.1** - **Cache hydration fix (77s → <2s boot)**: boot log po v3.38.0 odhalil `+79709ms cache loaded rendered=5000 msgCache=5699` — 77.8s synchronního main-thread blocku během `_loadCachedMessages`. Root cause: v3.24.24 bumpnutý `maxMessages` z 500 na 5000 znamenal rendering 5000+ DOM elementů in-row bez yield. Fix: (1) `maxMessages` default zpět na **500** + one-way migrace configů s > 500 zpět na 500 (v3.24.24 bump byl chyba v retrospektivě — UX hodnota "drž 5000 zpráv" ≠ 40-80s blocked boot). (2) `_loadCachedMessages` trimuje cache přes `.slice(-cap)` před renderem. (3) **Chunked insert** přes `requestIdleCallback` (fallback `setTimeout 0`) s CHUNK=40 — UI thread dýchá mezi batchi, první batch visible ~100ms, celé hydration ~500-800ms. **Aktuální verze**
