...
- **v3.38.24** - **Pin merge per-field (no more downgrades)**: log ukázal že Twitch flipuje pin card mezi expanded (plný DOM) a `__collapsed` (author footer unmounted) každou pár vteřin. Předtím DOM-wins logika při collapsed state downgradeovala banner (ztratil author+badges+time). Nový `_mergePinCard(dom, gql)` — per-field picker (DOM → last-good cache → GQL). `_lastGoodPinCache` se aktualizuje jen když DOM má complete footer (author + time + body). Tím: (1) jednou vidíme expanded data, "drží se" přes další collapse cykly; (2) emote URLs z DOM zůstávají i když se GQL fallback zapne; (3) GQL author/badges vyplní collapsed DOM mezery. Channel switch resetuje cache. **Aktuální verze**
