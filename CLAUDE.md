...
- **v3.38.7** - **Scroll-lock při append když user scrolloval pryč**: user reportoval že když scrolloval nahoru a nové zprávy stackovaly dole, chat se "stále posouval" místo aby zůstal na místě. Root cause: i s `overflow-anchor: none` Chrome occasionally nudge-uje `scrollTop` během reflow (scrollbar-gutter churn, image-load dimenze, atd.). Fix: v `_addMessage` capture `scrollTop` před `appendChild` a restore po appendu pokud `!autoScroll && !hydratingOlder`. Plus 50ms `_programmaticScrollUntil` suppression aby restore nebyl interpretován jako user scroll event. **Aktuální verze**
