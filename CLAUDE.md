...
- **v3.38.4** - **Hydrate scroll-restore stale-capture fix**: `prevHeight`/`prevTop` teď zachyceno AŽ PO `requestAnimationFrame` yieldu, ne před. Stale capture přes 16ms frame gap znamenal že když user pokračoval ve scroll během yieldu, restored scrollTop = stale_prevTop + delta neskočil na správné místo (vizuálně jako by zprávy byly out-of-order). Oprava: snapshot běží synchronně na stejném ticku jako prepend + restore, takže mezi capture a restore nemůže proklouznout žádný scroll event. **Aktuální verze**
