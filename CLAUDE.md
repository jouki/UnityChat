...
- **v3.38.23** - **Idempotent pin rerender + collapsed state persistence**: pin banner se každé 4s re-mountoval (tear down + rebuild), ztrácel user's click na chevron toggle a vypadal jako flicker. Fix: (1) hash visible card data (kind/text/pinId/author/color/badges/segments/time) — pokud matches předchozí render, **skip DOM re-mount** úplně. (2) Před teardown si uložíme per-pin `collapsed` state (key = `pin.pinId` / fallback `pinnedBy-author`), po rerender honor uživatelský click. User's expand toggle teď vydrží napříč polls. Plus doc: boot-time emote resolve fail je known limitace — anonymní GQL nevrací subscriber emotes (Client-Integrity gate) → pin text zobrazuje emote names; DOM scraper nebo IRC learning je doplní za pár sekund. **Aktuální verze**
