...
- **v3.38.6** - **Lazy-hydrate ordering & scroll-restore fix**: (1) `_loadCachedMessages` teď **sortuje `_msgCache` podle timestamp** (insertion order drift kvůli pozdě-příchozím scraped zprávám rozbíjela chronologii). (2) `_hydrateOlderMessages` sortuje slice před prependem (safety net). (3) `_addMessage` early-return před `_trim`/`_scroll`/`_cacheMsg` když `_hydratingOlder` (předtím hydrated msgs volaly `_cacheMsg` → push do konce cache → korupce orderu; plus `_trim` běžel proti fragmentu). (4) **Scroll-restore policy** — pokud user byl u úplného vrchu (prevTop < 40 px), po prependu `scrollTop = 0` (uvidí nově načtené starší zprávy); jinak tradiční restore (reading line fixed). **Aktuální verze**
