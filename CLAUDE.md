...
- **v3.38.10** - **Pin author: search INSIDE time container**: diagnostic Pin log z v3.38.8 odhalil real DOM: pin footer `<p>` má strukturu `[badges-span] <span class="chatter-name"><span class="CoreText"><span style="color:X">Name</span></span></span> odesláno v HH:MM` — author žije **UVNITŘ** time containeru, ne vedle něho. v3.38.9 `sibs.indexOf(timeEl) === -1` pokud timeEl je `<p>` (ne span/a) → walk-previous loop se přeskočil. Fix: dvoustupňový search — nejdřív leaf-first uvnitř timeEl, pak fallback na walk previous siblings. Plus `<p>/<div>` akceptovaný jako timeEl (předtím jen span/a). **Aktuální verze**
