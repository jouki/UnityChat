...
- **v3.38.11** - **Pin highlight diagnostic pro collapsed chat**: když user má vanilla Twitch chat collapsed (native Sbalit nebo náš hide režim), Twitch nevygeneruje `.community-highlight-stack__card` — místo toho mini "!N" badge na playeru. UC banner neukázal nic. Rozšířené `HIGHLIGHT_SELECTORS` o `[class*="community-highlights"]`, `[data-a-target="community-highlights"]`, `.pinned-chat__highlight-card`. Plus rate-limited diagnostic (once/30s) — když `snapshotHighlights` nenajde nic ale DOM obsahuje pin/highlight-related classes, dumpne outerHTML prvních 6 kandidátů jako `HighlightDiag` log pro tuning selektorů další iterace. **Aktuální verze**
