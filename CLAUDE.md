...
- **v3.38.8** - **Pin extractor přepsán + DOM diagnostic**: Twitch nový pin layout rozbíjel `extractPinDetails`. (1) `pinnedBy` teď DOM-walk label span → next username sibling; regex fallback `[\p{L}\p{N}_]+` (ne `\S+`, co zabíral "MeewileDONO:"). (2) Body selector fallback odmítne elementy co **obsahují** header label anywhere (ne jen "starts-with" check) + odmítne pinnedBy-only + time-only. (3) Author badges scope: walk up k flex/grid row místo card-wide. (4) Author null-guard: když DOM má jen jeden username a ten je pinnedBy, necastovat ho na author. Plus **diagnostic log** — když extract missuje author/body/pinnedBy, pošle outerHTML (trimmed 4000 chars) do `UC_LOG` jako `Pin` tag → capture v DIAG dumpu pro budoucí tuning. **Aktuální verze**
