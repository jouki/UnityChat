...
- **v3.38.12** - **Auto-expand collapsed pin card**: Pin diagnostic dump ukázal `pinnedBy=✓ body=9 segs ✓ author=null badges=0 timeText=null` na Cohh pinu → důvod: Twitch v collapsed stavu author footer vůbec neposkytuje. Fix: content script detekuje `__collapsed` class + klikne expand button (real-event sequence pointerdown/mouseup/click) před `extractPinDetails` — Twitch card expanduje, author row se mountne, extractor dostane full data. `_ucExpanded` flag prevent re-click. Sidepanel render už byl graceful (footer skip pokud `!author && !timeText`). **Aktuální verze**
