...
- **v3.38.22** - **Pin duplication fixes (2 bugy)**: (1) **Stacking duplicity**: `_rerenderHighlights` merguje cache+GQL, ale `_handleHighlights` pak `_lastDomHighlightCards = merged` přepisoval cache na merged výsledek → každý 4s tick přidal kopii pinu → po ~1 min stack 15+ duplikátů. Fix: `_lastDomHighlightCards` drží **jen non-pin cards** (filter), plus `_rerenderTag` identity → rerender msgs skipují caching block. (2) **Mod badge 2×**: DOM `extractPinDetails` authorRow walkup dosahl card-root flex containeru → zachytil pinner's mod badge z headeru + author 3 badges z footeru = 4 (mod duplicitní). Fix: `authorRow = authorEl.closest('p')` úzký scope na pin footer `<p class="jPfhdt">` + hard dedup na badge src URL. **Aktuální verze**
