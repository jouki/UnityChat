...
- **v3.38.5** - **Credits pill: stale icon + watch-streak filter**: (1) při channel switch na kanál bez custom channel-points icon `_lastPointsIcon` zůstal nastavený ze starého channelu (render path neclearoval `backgroundImage`). Fix: explicit else-branch vyčistí `pointsIcon.style.backgroundImage = ''` a `.has-icon` class když `_lastPointsIcon` je null. Plus `data.pointsIcon === null` znamená reset (předtím falsy check chybně přeskočil null). (2) Twitch občas zrcadlí watch-streak / sub-streak widget do points-balance slotu (text není číslo) — filter `/^[\d\u00A0\s.,]+(?:\s*(?:tis|k|mil|m)\.?)?$/i` dropne non-balance text, keep-last-shown brání rendruje garbage. Rejected values loggují jako `StreakSkip` pro diagnostiku. **Aktuální verze**
