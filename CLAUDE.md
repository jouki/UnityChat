...
- **v3.38.15** - **GQL schema fix pro FETCH_PINS**: diagnostic log z v3.38.14 odhalil že `PinnedChatMessage` type **nemá** direct fields `sender`, `senderBadges`, `content`, `pinnedAt` — to byl můj špatný schema guess. Skutečné pole jsou nested v `message` subselection: `node.message.sender { chatColor }`, `node.message.content.fragments`. Plus `pinnedAt → startsAt`, `Emote.emoteID → id`, `Cheermote` má jen `prefix`. Query přepsaná na real schema, mapper aktualizovaný. Badges not available přímo na PinnedChatMessage — fallback přes `_twitchBadges` cache via sender login v sidepanelu. **Aktuální verze**
