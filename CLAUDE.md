# UnityChat - Chrome Extension + Backend v3.40.28
...
- **v3.40.26–28** - (26) Kick badge po echu vlastní zprávy nemizí (`_badgeEntry(platform, key)`). (27) Nová zpráva plynule přijede zespodu (`core/slide-in.js`, CSS proměnná `--uc-slide`, 160 ms; určeno pro OBS chat, v addonu a na webu zkušebně). (28) **Odpovědi napříč platformami pro všechny v UnityChatu**: klient nahlásí `ucReplyTo` (`/chat/send`) / `replyTo` (`/chat/uc-sent`), server spáruje s echem (`ucReplies` v `lib/ucSends.ts`), uloží `content_raw.ucReply`, klienti dostanou `replyTo {platform, uc:true}` v historii, `/chat/stream` a SSE `uc-reply`; ↩ s logem platformy citované zprávy, úvodní `@jméno` v UnityChatu skryté (`core/uc-reply.js`). Backend: „Odhlásit se“ odhlásí všechny platformy účtu (`signOutAccount`, `web_identities.signed_out_at`).
- **v3.40.25** - **Zprávy přes přihlášený účet (addon = web)**: addon posílá účtem uživatele přes `POST /chat/send` (Twitch Helix / Kick API / YouTube insert), platformu volí uživatel v menu „Psát jako“ u pole (`uc_send_platform`), bez přihlášení je místo pole výzva + přihlašovací okno se třemi logy. Sdílené s webem: `core/login-modal.js` (`LoginModal`, `renderPlatformMenu`) + `extension/composer.css`. Sekce účtu v nastavení a ikona v hlavičce zrušeny. Řádek bodů/bitů Twitche jen s přihlášeným Twitch účtem. **Záloha:** Dev mode → „Posílat přes otevřenou kartu (stará cesta)“ (`config.legacyTabSend`) vrátí původní odesílání přes tab i výběr platformy podle tabu. Echo páruje `_contentKey` (YouTube = handle z `login`, ne název kanálu). UC_LOG `Send`, `Account`.
