...
- **v3.38.21** - **emoteID gated by Client-Integrity — resolve by name**: log ukázal "Cannot query field emoteID on Emote". User ji **v browseru** dostane (Twitch UI má integrity token), náš anonymní fetch ne → celá query failovala, data chyběla. Fix: query zahodí `... on Emote { emoteID }` spread, pulluje jen `__typename`. Sidepanel resolve emote URL přes text name proti local emote library (Twitch native / 7TV channel+global / BTTV / FFZ). Unknown emote → readable text fallback. Plus `sentAt`, `sender`, `displayBadges` teď projdou (byly už v GQL, jen query failovala jako celek). Poll interval 8s → 4s + `visibilitychange` trigger (immediate tick na panel focus). **Aktuální verze**
