...
- **v3.38.9** - **Pin extractor iter 2 — short-leaf pinnedBy + time-anchored author**: (1) `pinnedBy` teď hledá **short leaf element** (textContent <60 chars) co matchuje `"Připnuto uživatelem X"` — dlouhé parent elementy obsahují i body text ("MeewileDONO:"), jen pure header span je krátký. (2) Author detection secondary heuristic: najít element s textem "odesláno v …", pak walk previous siblings pro short colored username span → chytí Twitch pin footer layout i bez `.seventv-chat-user` tříd. (3) Badges fallback: pokud author nalezen ale bez badges, scan preceding siblings (+ jejich descendants) authorEl v DOM order — chytí badge imgs vlevo od autora v pin footer. **Aktuální verze**
