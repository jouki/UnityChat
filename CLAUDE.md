...
- **v3.38.13** - **Hide strategie: width:0 → position:fixed off-screen**: správná diagnóza hide-not-collapse bugu. `width: 0` triggerovalo Twitch interní ResizeObserver co prohlásil chat za "collapsed" → Twitch přepnul pin cards do compact render (bez author footeru), highlights do mini "!N" badge. Řešení: `position: fixed; right: -9999px; height: 100vh; visibility: hidden; pointer-events: none`. Column je mimo viewport ale `offsetWidth` zůstává na natural hodnotě — Twitch collapse detection nefire-uje, pin cards jsou renderované full, highlights jako normal cards. V3.38.12 expand-button workaround zůstává jako backup (zbytečný pro náš hide, aktivuje se jen když user má vanilla "Sbalit"). **Aktuální verze**
