...
- **v3.38.3** - **Lazy-load UX polish**: (1) inline `.hydrate-spinner` při `_hydrateOlderMessages` — brand-orange gradient card s rotating ring + "Načítání starších zpráv…" label, pulse glow. Vsazený do chatEl, takže scrollHeight delta match aligned s konečnou pozicí po remove. `requestAnimationFrame` yield před renderem aby spinner dostihl paint. (2) batch size **250 → 150** per scroll-up trigger (plynulejší odezva na slabších strojích). (3) **unread counter/separator suppression** během `_hydratingOlder` — prepend starých zpráv už není "nová zpráva" (fix user-reportovaného "250 nových zpráv" omylu). (4) **CS plural rules** na "↓ N nových zpráv" pill — helper `_formatNewMsgCount(n)`: 1 → singular (`nová zpráva`), 2-4 → few (`nové zprávy`), 0/5+ → many (`nových zpráv`). **Aktuální verze**
