...
- **v3.38.2** - **Lazy scroll-up load (storage 5000, DOM jen 250)**: user chce zachovat 72h historii tisíců zpráv, ale boot musí zůstat rychlý. Split mezi **storage cap** (`maxMessages: 5000` — kolik držíme v `_msgCache` + stringifujeme do storage) a **render cap** (`initialRender: 250` — kolik naRenderujeme při bootu). Nový `_hydratedIdx` kurzor + `_hydrateOlderMessages()` — když user scrolluje do top 200px, prepend dalších 250 z `_msgCache` + scroll-position preservation (zachytí `scrollHeight` delta). `_trim()` bumpne `_hydratedIdx` o odpovídající počet když DOM překročí cap. Channel switch + btn-clear-cache resetují `_hydratedIdx = 0`. Boot trvání: cache load ~500-800ms (250 msgs chunked). **Aktuální verze**
