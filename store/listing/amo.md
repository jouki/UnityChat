...
## 4. Poznámky pro recenzenta (EN, zkopírovat) — verze 3.41.0
...
2. Reading chat needs no login. To write, click "Pro psaní se musíš přihlásit"
   under the chat and sign in with Twitch, Kick or YouTube (standard OAuth page of
   the platform). Messages are then sent with that account through our backend
   using the platforms' official APIs (Twitch Helix chat/messages, Kick chat API,
   YouTube liveChatMessages.insert).
3. What's new in 3.41.0: sign-in and sending with the user's account (above),
   replies across platforms, chat sound effects (a music-note button that posts a
   "!se <name>" chat command; unlocked by the streamer's rewards), and a donation
   form with a bank QR code, which is still behind the "Dev mode" setting and not
   yet offered to users. The extension never handles money: it only shows payment
   details for a bank transfer made in the user's own banking app.

Why the permissions are needed:
- identity: signing in to our backend via the platform's OAuth page
  (launchWebAuthFlow); the password is typed on the platform's own site. Only our
  session token is stored locally; signing out removes it.
- cookies: reads the Twitch "auth-token" and Kick "session_token" cookies only to
  authenticate requests to Twitch's and Kick's own APIs for actions done in the
  platform page (pinning, the fallback sending path, detecting the logged-in Kick
  name). They are never stored and never sent anywhere other than Twitch/Kick.
- scripting (world: MAIN) and tabs: find the open stream tab and use the
  platform's own page APIs that require the page's session (user cards, pins,
  fallback sending).
...
- Host permissions: the three platforms, emote/badge providers (7TV, BTTV, FFZ,
  IVR) and our backend api.jouki.cz (sign-in, sending messages, chat history,
  shared nicknames, sound effects, donations).
...
before insertion; emote/badge URLs come from fixed CDN templates. The donation
QR code is an SVG generated by our own server from the payment string.

Data collection is declared in the manifest (data_collection_permissions) and
described at https://jouki.cz/UnityChat/privacy (updated 25 September 2026).
```
